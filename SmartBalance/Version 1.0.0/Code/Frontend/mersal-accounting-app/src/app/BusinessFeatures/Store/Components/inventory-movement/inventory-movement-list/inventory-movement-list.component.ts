import { InventorySearch, InventoryMovementSearch } from './../../../Models/inventory-search.model';
...
@Component({
  ...
  styleUrls: ['./inventory-movement-list.component.css']
}) 
export class InventoryMovementListComponent implements OnInit {
   ...
   filter: any;
   postSearch: InventoryMovementSearch = new InventoryMovementSearch();
   inventoryList: any[];InventoryMovementSearch
   inventory: number;
   inventoryId: any;
  ...
  Inventories: any[];
   ...
   dataStateChange(state: DataStateChangeEvent): void {
     ...
     if (this.filter.length > 0) {
       ...
       this.postSearch = new InventoryMovementSearch();
     }
   ...
   public pageChange(event: PageChangeEvent): void {
    ...
    let url = [`/store/inventory/list/${this.pageIndex}`];
   ...
   inventoryFromChange(event){
    ...
    inventoryMovementTypeChange(event){
      //debugger;
      this.pageIndex=0;
      this.postSearch.inventoryMovementTypeId=event.value;
      this.inventoryMovementType=this.postSearch.inventoryMovementTypeId;
      this.getList();
     }
   ...
   getList() {
     ...
     this.serv.getInventoryMovementList(this.postSearch)
       .subscribe(res => {
         debugger;
