...
export class InventoryListComponent implements OnInit {
  ...
  dataStateChange(state: DataStateChangeEvent): void {
    ...
    if (this.filter.length > 0) {
      for (var _i = 0; _i < this.filter.length; _i++) {
        ...
        if (this.filter[_i].field == "nameAr") {
          this.postSearch.nameAr = this.filter[_i].value;
        }

        if (this.filter[_i].field == "nameEn") {
          this.postSearch.nameEn = this.filter[_i].value;
        }
  ...
  public pageChange(event: PageChangeEvent): void {
    ...
    let url = [`/store/inventory-editor/list/${this.pageIndex}`];
  ...
  getList() {
    ...
    //debugger
    this.inventoryService.getInventorysByPost(this.postSearch)
      .subscribe(res => {
