...
import { User, UserGroupList } from '../../Models/user.model';
...
export class UserGroupsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userService.setActiveTab("userGroups");
  ...
  extractRouteParams(): void {
    ...
    if (this.id) {
      this.getAllUnSelectedGroupsForUser(this.id);
      this.getUserGroups(this.id);
  ...
  getAllUnSelectedGroupsForUser(userId: number) {
    this.isProccessing = true;
    this.groupService.getAllUnSelectedGroupsForUser(userId)
      .subscribe(res => {
        debugger;
  ...
  getUserGroups(userId: number) {
    this.userService.getUserGroups(userId)
    .subscribe(res => {
  ...
  save() {
    if(this.hasChange) {
      this.isProccessing = true;
      let model: UserGroupList = new UserGroupList();
      model.userId = this.id;
      model.list = this.changedDestList;

      this.userService.updateUserGroups(model)
      .subscribe(res => {        
        this.notificationService.showOperationSuccessed();
        this.hasChange=false;
        this.isProccessing=false;
        this.gotoUserMenuItems();
      },
      (error) => {
        this.isProccessing=false;
