...
export class ListComponent implements OnInit {
  ...
  getIsClosedDropDownData() {
    if(this.language == "en") {
      this.isClosedDropDownData = [
        {text: "All", value: null},
        {text: "Yes", value: true},
        {text: "No", value: false},      
      ];
  ...
  isClosedChange(event: any) {
    debugger;
    if(event) {
      this.postSearch.isClosed = event.value;
      this.getList();
    }
  }

  // closedMonthkChange(event) {
  //   //debugger;
  //   this.pageIndex = 0;
  ...
  dataStateChange(state: DataStateChangeEvent): void {
    ...
    if (this.filter.length > 0) {
      for (var _i = 0; _i < this.filter.length; _i++) {
        ...
        if (this.filter[_i].field == "month") {
          this.postSearch.month = this.filter[_i].value;
        }
        ...
        if (this.filter[_i].field == "isClosed") {
          this.postSearch.isClosed = this.filter[_i].value;
        }
  ...
  public pageChange(event: PageChangeEvent): void {
    ...
    let url = [`/closed-month/list/${this.pageIndex}`];
  ...
  getList() {
    ...
    //debugger
    this.closedMonthService.getClosedMonthsByPost(this.postSearch)
      .subscribe(res => {
