...
import { PurchasesProductService } from '../../services/purchases-product.service';

@Component({
  selector: 'app-purchases-product',
  templateUrl: './purchases-product.component.html',
  styleUrls: ['./purchases-product.component.scss'],
  animations: [routerTransition()],
  providers: [PurchasesProductService]
...
export class PurchasesProductComponent implements OnInit {
  ...
  constructor(private translate: TranslateService, public router: Router, private fb: FormBuilder,
    private purchasesServ: PurchasesProductService) {
  ...
  getAllBillTypes() {
    this.BillTypes = this.purchasesServ.getAllBillTypes();
  }
  AddBill() {
    this.Bill.push({
      id: 2,
